...
{
    public abstract class DataScriptEditor : UnityEditor.Editor
    {
        ...
        public void Rebuild(ClassBuilder builder, string json, string rawJson)
        {
            _tryRebuild = false;
            var type = SubClassType;
            builder.GenerateClasses(type.Namespace, DataScriptMaker.GetScriptDirectory(builder.DataType));
            Debug.Log($"Rebuild Done. ({SheetName})");
        }

        public void DownLoad(ClassBuilder builder, string json, string rawJson)
        {
            SetAssetData(json);
