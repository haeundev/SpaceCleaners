...
namespace LiveLarson.Camera.Auto_Letterbox
{
    /* ForceCameraRatio.cs
     *
    ...
    public class CameraRatio
    {
        ...
        public CameraRatio(UnityEngine.Camera _camera, Vector2 _anchor)
        {
            camera = _camera;
        ...
        {
            vectorAnchor = _anchor switch
            {
                CameraAnchor.Center => new Vector2(0.5f, 0.5f),
                CameraAnchor.Top => new Vector2(0.5f, 1f),
                CameraAnchor.Bottom => new Vector2(0.5f, 0f),
                CameraAnchor.Left => new Vector2(0f, 0.5f),
                CameraAnchor.Right => new Vector2(1f, 0.5f),
                CameraAnchor.TopLeft => new Vector2(0f, 1f),
                CameraAnchor.TopRight => new Vector2(1f, 1f),
                CameraAnchor.BottomLeft => new Vector2(0f, 0f),
                CameraAnchor.BottomRight => new Vector2(1f, 0f),
                _ => vectorAnchor
            };
        }
        ...
        public void CalculateAndSetCameraRatio(float _width, float _height, bool _horizontalLetterbox)
        {
            ...
            var screenViewPortHorizontal = new Rect();
            var screenViewPortVertical = new Rect();
            ...
            screenViewPortHorizontal.y = Mathf.Lerp(originViewPort.y,
                originViewPort.y + (originViewPort.height - screenViewPortHorizontal.height), vectorAnchor.y);
            ...
            screenViewPortVertical.x = Mathf.Lerp(originViewPort.x,
                originViewPort.x + (originViewPort.width - screenViewPortVertical.width), vectorAnchor.x);
            screenViewPortVertical.y = originViewPort.y;

            // Use the best fitting of the two
            if (screenViewPortHorizontal.height >= screenViewPortVertical.height &&
                screenViewPortHorizontal.width >= screenViewPortVertical.width)
            {
                if (screenViewPortHorizontal.height <= originViewPort.height &&
                    screenViewPortHorizontal.width <= originViewPort.width)
            ...
            {
                if (screenViewPortVertical.height <= originViewPort.height &&
                    screenViewPortVertical.width <= originViewPort.width)
                    camera.rect = screenViewPortVertical;
                else
                    camera.rect = screenViewPortHorizontal;
            }
    ...
    public class ForceCameraRatio : MonoBehaviour
    {
        ...
        public Color letterBoxCameraColor = new(0, 0, 0, 1);

        public List<CameraRatio> cameras;

        public UnityEngine.Camera letterBoxCamera;
        ...
        public void Init()
        {
            ...
            else if (cameras == null || cameras.Count == 0) cameras = new List<CameraRatio>();

            ValidateCameraArray();

            // Set the origin viewport space for each Camera
            for (var i = 0; i < cameras.Count; i++) cameras[i].ResetOriginViewport();

            // If requested, a Camera will be generated that renders a letter box Color
            if (createCameraForLetterBoxRendering)
            {
                letterBoxCamera = new GameObject().AddComponent<UnityEngine.Camera>();
                ...
                for (var i = 0; i < cameras.Count; i++)
                    if (cameras[i].camera.depth == -100)
                        Debug.LogError(cameras[i].camera.name +
                                       " has a depth of -100 and may conflict with the Letter Box Camera in Forced Camera Ratio!");
            }

            if (forceRatioOnAwake) CalculateAndSetAllCameraRatios();
        }
        ...
        /// <returns></returns>
        private CameraRatio GetCameraRatioByCamera(UnityEngine.Camera _camera)
        {
        ...
        {
            var allCameras = FindObjectsOfType<UnityEngine.Camera>();
            cameras = new List<CameraRatio>();

            for (var i = 0; i < allCameras.Length; i++)
                if (createCameraForLetterBoxRendering ||
                    allCameras[i] != letterBoxCamera) // Ignore the Custom LetterBox Camera
        ...
        public void CalculateAndSetAllCameraRatios()
        {
            ...
            if (currentAspect > targetAspect) horizontalLetterbox = false;
        ...
        /// <param name="_camera"></param>
        /// <param name="_anchor"></param>
        public void SetCameraAnchor(UnityEngine.Camera _camera, Vector2 _anchor)
        {
            var camera = GetCameraRatioByCamera(_camera);
            if (camera != null) camera.vectorAnchor = _anchor;
        }

        public CameraRatio[] GetCameras()
        {
            if (cameras == null) cameras = new List<CameraRatio>();
