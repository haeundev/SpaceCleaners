...
namespace SoftMasking
{
    ...
    public class SoftMask : UIBehaviour, ISoftMask, ICanvasRaycastFilter
    {
        ...
        public SoftMask()
        {
            var materialReplacer = new MaterialReplacerChain(MaterialReplacer.globalReplacers, new MaterialReplacerImpl(this));
        ...
        /// </summary>
        public Shader defaultShader { get => _defaultShader;
        ...
        /// </summary>
        public Shader defaultETC1Shader { get => _defaultETC1Shader;
        ...
        /// </summary>
        public float raycastThreshold { get => _raycastThreshold;
        ...
        protected virtual void LateUpdate()
        {
            ...
            if (maskingEnabled)
            {
                ...
                FindGraphic();
                if (_lastMaskRect != maskTransform.rect || !ReferenceEquals(_graphic, prevGraphic))
                    _dirty = true;
            }
        ...
        static float ClampPixelsPerUnitMultiplier(float value) { return Mathf.Max(value, 0.01f); }
        ...
        void OnTransformChildrenChanged() { SpawnMaskablesInChildren(transform); }
        ...
        void UnsubscribeFromWillRenderCanvases() { Canvas.willRenderCanvases -= OnWillRenderCanvases; }
        ...
        RectTransform maskTransform => _maskTransform ? _maskTransform : (_maskTransform = _separateMask ? _separateMask : GetComponent<RectTransform>());
        ...
        void ISoftMask.ReleaseReplacement(Material replacement) { _materials.Release(replacement); }

        void ISoftMask.UpdateTransformChildren(Transform transform) { SpawnMaskablesInChildren(transform); }
        ...
        void SpawnMaskablesInChildren(Transform root)
        {
            using (new ClearListAtExit<SoftMaskable>(s_maskables))
                for (int i = 0; i < root.childCount; ++i)
                {
                    ...
                    if (s_maskables.Count > 1)
                    {
                        var list = s_maskables.ToList();
                        list.ForEach(Destroy);
                        s_maskables.Clear();
                    }

                    Assert.IsTrue(s_maskables.Count <= 1, $"{child.name} has {s_maskables.Count} SoftMaskable.");
                    if (s_maskables.Count == 0)
                        child.gameObject.AddComponent<SoftMaskable>();
                }
        }

        void InvalidateChildren() { ForEachChildMaskable(x => x.Invalidate()); }

        void NotifyChildrenThatMaskMightChanged() { ForEachChildMaskable(x => x.MaskMightChanged()); }
        ...
        void DestroyMaterials() { _materials.DestroyAllAndClear(); }
        ...
        public static bool IsImageTypeSupported(Image.Type type) { return type == Image.Type.Simple || type == Image.Type.Sliced || type == Image.Type.Tiled; }
        ...
        void CalculateSolidFill() { CalculateTextureBased(null, DefaultUVRect); }
        ...
        Matrix4x4 WorldToMask() { return maskTransform.worldToLocalMatrix * canvas.rootCanvas.transform.localToWorldMatrix; }
        ...
        static class Mathr
        {
            ...
            public static Vector4 BorderOf(Vector4 outer, Vector4 inner) { return new Vector4(inner.x - outer.x, inner.y - outer.y, outer.z - inner.z, outer.w - inner.w); }

            public static Vector4 ApplyBorder(Vector4 v, Vector4 b) { return new Vector4(v.x + b.x, v.y + b.y, v.z - b.z, v.w - b.w); }
            ...
            public static bool Inside(Vector2 v, Vector4 r) { return v.x >= r.x && v.y >= r.y && v.x <= r.z && v.y <= r.w; }
        }
        ...
        struct Diagnostics
        {
            ...
            static bool AreCompeting(SoftMask softMask, SoftMask other)
            {
                ...
                return softMask.isMaskingEnabled && softMask != other && other.isMaskingEnabled && softMask.canvas.rootCanvas == other.canvas.rootCanvas && !SelectChild(softMask, other).canvas.overrideSorting;
            }
