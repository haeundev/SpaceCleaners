...
public class GrappleEffect : MonoBehaviour
{
    ...
    [Header("Spiral")] [Tooltip("The strength of the spiral.")]
    ...
    [Tooltip(
        "The amount the horizontal part of the spiral is offset along 'Curve.' \nIf 'Curve' is a sine wave, 0.25 will result in a perfect spiral.")]
    public float HorizontalOffset = 0.25f;

    [Header("Noise")] [Tooltip("The strength of the noise.")]
    ...
    [Header("Curves")]
    [Tooltip(
        "The base curve that the points will be offset along. \nA sine wave will make it look like a grapple spiral.")]
    public AnimationCurve Curve = new();

    [Tooltip("The strength of the spiral and noise over time.")]
    public AnimationCurve MagnitudeOverTime = new();

    [Tooltip("The strength of the spiral and noise from the start to current end point within a 0 to 1 range.")]
    public AnimationCurve MagnitudeOverDistance = new();

    [Tooltip(
        "The vertical offset applied in worldspace to the line from the start to current end point within a 0 to 1 range.")]
    public AnimationCurve GravityOverDistance = new();

    [Tooltip("The strength of the gravity over time.")]
    public AnimationCurve GravityOverTime = new();

    private float scaledTimeOffset;
    private float spiralTimeOffset;
    ...
    {
        if (lineRenderer.enabled = Physics.Raycast(transform.position, transform.forward, out var hit))
            DoGrapple(hit.point);
    }
    ...
    private void Update()
    {
        ...
        for (var i = 0; i < lineRenderer.positionCount; i++)
        {
            ...
            position += Vector3.up *
                        (GravityOverDistance.Evaluate(t) * GravityOverTime.Evaluate(scaledTimeOffset) * Gravity);
